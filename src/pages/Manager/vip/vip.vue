<template>
	<view class="pages user">
		...
		<view class="customer-body clearfix" style="padding-left: 10px;padding-right: 10px;background-color: #FFFFFF;">
			<view class="customer-list21" v-for="(v,i) of list1" :key="i" @click="goToCoustomList(v.path, i)">
				<view class="customer-list">
					<view class="customer-icon-title">
						<image class="image1" :src="v.icon?v.icon:'/static/images/default_logo_1x1.jpg'" mode="aspectFit" />
						<text style="margin-left: 10px;">{{v.name}}{{v.value}}</text>
					</view>
					<image class="right-image" src="/static/images/icon_right1.png" mode="aspectFit" />
				</view>
				<view class="line"></view>
			</view>
		</view>
			...
			<view class="customer-list2" v-for="(v,i) of customerList" :key="i" @click="goToDetail(v.userNumber, i)">
				<view class="customer-icon-title2">
					...
					<view class="out-view2">
						...
						<view class="top-view2">
							<text class="text12">{{v.lastOrderTime}}</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goToCoustomList(path, i) {
				switch (i) {
					case 0:
						this.type = "potentialCustomer"
						break
					case 1:
						this.type = "newCustomer"
						break
					case 2:
						this.type = "repeatCustomer"
						break
					case 3:
						this.type = "lossCustomer"
						break
				}
				uni.navigateTo({
			...
			init() {
				this.getustomerAnalysis()
				this.getUserList()
			},
			//会员数据统计
			getustomerAnalysis() {
				uni.request({
					url: baseurl + "shopAnalysis/customerAnalysis/" + this.shopId, //仅为示例，并非真实接口地址。
					method: 'GET',
					header: {
					...
					success: (res) => {
						...
						if (res.data.code == 0) {
							this.avgPayment = res.data.result.avgPayment
							this.customerCount = res.data.result.customerCount
							this.lossCustomerCount = res.data.result.lossCustomerCount
							this.newCustomerCount = res.data.result.newCustomerCount
							this.newRepeatBuyerCount = res.data.result.newRepeatBuyerCount
							this.potentialCustomerCount = res.data.result.potentialCustomerCount
							this.repeatBuy = res.data.result.repeatBuy

							this.managerMenu[0].value = this.customerCount + "位"
							this.managerMenu[1].value = this.repeatBuy + ""
							this.managerMenu[2].value = "￥" + util.moneyFormat(this.avgPayment) + ""

							this.list1[0].value = " " + this.potentialCustomerCount + " 人"
							this.list1[1].value = " " + this.newCustomerCount + " 人"
							this.list1[2].value = " " + this.newRepeatBuyerCount + " 人"
							this.list1[3].value = " " + this.lossCustomerCount + " 人"
						}
					}
				});
			},
					...
					success: (res) => {
						...
						if (res.data.code == 0) {
							for (let i = 0; i < res.data.result.list.length; i++) {
								if (res.data.result.list[i].lastOrderTime == null) {
									res.data.result.list[i].lastOrderTime = ""
								} else {
									res.data.result.list[i].lastOrderTime = util.timeago(res.data.result.list[i].lastOrderTime * 1000)
								}
							}
